...
class WeightCacheOpt : public torch::CustomClassHolder
{
private:
    ...
    void rebuild_tensor_views()
    {
        ...
        active_ups_cache = up_tensor.to(current_device);
...
public:
    ...
    void init(const torch::Tensor &init_mask, int64_t hidden_size,
              const torch::Tensor &up_weight, const torch::Tensor &down_weight)
    {
    ...
    torch::Tensor get_active_up_weight() const
    {
        TORCH_CHECK(cache_valid, "Cache is not valid");
        return active_ups_cache;
    }
