...
from src.activation_capture import ActivationCapture, ACTIVATION_CAPTURE
...
class ContextualSparsityAnalyzer:
    ...
    def __init__(self, model, tokenizer, device):
        ...
        self.capture = ACTIVATION_CAPTURE[model.config.model_name]()
        self.capture.register_hooks(model)
        self.num_layers = self.capture.get_layers(model)
...
def analyze_sparsity(args, model_name, device):
    ...
    try:
        ...
        for key, layer_sparsities in analyzer.mlp_sparsity:
...
def plot_sparsities(args, device):
    ...
    for model in args.models:
        ...
        for k, v in model_sparsities:
            outs[k][model_name] = v
    
    for k, outs_k in outs.keys():
        plt.figure(figsize=(10, 6))
        max_layers = 0
        for model_name, model_sparsities in outs_k.keys():
            max_layers = max(max_layers, len(model_sparsities))
        ...
        plt.legend()
        plt.xticks(max_layers[::4], [f'L{i}' for i in max_layers[::4]])
        plt.minorticks_on()
